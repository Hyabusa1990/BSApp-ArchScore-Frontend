...
The API contract lives in `docs/Fawkes-OpenApi.json` — a vendored (manually refreshed) copy of the OpenAPI spec from the backend repo, the single source of truth shared with the C# backend. Source: https://github.com/Hyabusa1990/BSApp-ArchScore-Backend/blob/main/Fawkes-OpenApi.json — see `docs/README.md` for provenance and how to refresh it. (The former `ArchScore-SpecsAndDocu` git submodule, a separate repo with `openapi.yaml`, was removed — that file no longer exists upstream.)

**Staleness check (do this at the start of any session touching the API)**: since this is a manual snapshot with no auto-sync, at the start of any session that touches `src/lib/api/*.ts`, `src/mocks/handlers/*.ts`, or `docs/Fawkes-OpenApi.json` itself, check whether a newer spec exists upstream before treating the local copy as current:

```bash
gh api repos/Hyabusa1990/BSApp-ArchScore-Backend/commits?path=Fawkes-OpenApi.json --jq '.[0].sha'
```

Compare the result against the commit SHA recorded in `docs/README.md`. If they differ, tell the user a newer spec is available and ask whether to refresh `docs/Fawkes-OpenApi.json` (command in `docs/README.md`) before relying on it — don't refresh silently, since a spec change may require matching code changes.
