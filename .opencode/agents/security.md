...
description:
  ...
  Issue-Verwaltung via Forgejo MCP. Alle nutzerseitigen Ausgaben auf Deutsch.
...
> **Ausgabesprache:** Schreibe **alle nutzerseitigen Ausgaben auf Deutsch** — den Chat-Bericht,
> jeden Issue-Titel, jeden Issue-Body und jeden Kommentar. Die Anweisungen unten sind auf Englisch
> für Präzision; die Lieferergebnisse sind deutsch. Wechsle nicht ins Englische.
...
1. `git remote get-url origin` → Repo-Owner und Repo-Name extrahieren (für MCP-Calls)
...
6. Über den forgejo MCP-Server: alle offenen Issues mit Label `security`
...
10. Für jeden KRITISCH/HOCH-Fund:
    - Existiert bereits ein Issue → Kommentar mit aktuellem
      Commit-Hash hinzufügen
    - Kein Issue vorhanden → neues Issue erstellen (Format siehe unten)
